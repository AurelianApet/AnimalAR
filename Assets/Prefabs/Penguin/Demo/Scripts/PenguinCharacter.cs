...
public class PenguinCharacter : MonoBehaviour {
	...
	public void Move(){
        penguinAnimator.SetFloat("Forward", forwardSpeed);
        penguinAnimator.SetFloat("Turn", yawVelocity);
        if (isSwimming)
        {
            penguinAnimator.SetFloat("UpDown", forwardSpeed);
            transform.RotateAround(transform.position, transform.up, Time.deltaTime * yawVelocity * 100f);

            penguinRigid.velocity = (transform.up * forwardSpeed + transform.forward) * swimSpeed;
